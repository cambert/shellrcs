...
;; String with default arguments to pass to "p4 annotate".
;; This default asks for changes instead of file revisions in the
;; annotation.
(defcustom p4-default-annotate-options "-c"
  "Type of annotation to be displayed: -c for changes instead of revisions."
  :type 'string
  :group 'pr
)
  ...
  (let (p4-client-root pmin)
    (save-excursion
	  ...
	  (progn
	    ...
	    ;;; PWS: attempt to handle AltRoots.
	    (if (and (not (file-directory-p p4-client-root))
		     (re-search-forward "^AltRoots:" nil t))
		(while (and (not found)
			    (progn
			      (forward-line 1)
			      (looking-at "^[ \t]+\\([^ \t].*\\)$")))
		  (let ((tmp-root
			 (p4-canonize-client-root
			  (match-string 1))))
		    (if (file-directory-p tmp-root)
			(setq found t
			      p4-client-root tmp-root))
		    )))
	    (delete-region pmin (point-max)))))
    p4-client-root))
...
;; The p4 annotate command
(defp4cmd p4-annotate ()
  "annotate"
  "To provide an annotation of the current file, type \\[p4-annotate].\n"
  (interactive)
  (let ((args (p4-make-list-from-string p4-default-annotate-options)))
    (if (p4-buffer-file-name-2)
	(setq args (append args
			   (list (p4-buffer-file-name-2)))))
    (if current-prefix-arg
	(setq args (p4-make-list-from-string
		    (p4-read-arg-string "p4 annotate: "
					p4-default-annotate-options))))
    (p4-noinput-buffer-action "annotate" nil 's args))
)
